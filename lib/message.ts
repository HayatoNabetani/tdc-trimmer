...
export function buildMessage(
  ...
  // 8.1 / 8.2.1 宿泊
  const isAfternoon = input.pickupTime === 'afternoon';
  const lines: string[] = [
    '【お見積もり】',
    `🐶 ワンちゃんのサイズ：${sizeLabel}`,
    `📅 ご利用：宿泊 ${result.label}（${isAfternoon ? '午後' : '午前'}お迎え）`,
  ];

  if (input.checkIn) lines.push(`　チェックイン：${jpDate(input.checkIn)}`);
  if (input.checkOut) {
    lines.push(
      `　チェックアウト：${jpDate(input.checkOut)}（${
        isAfternoon ? '午後' : '午前'
      }お迎え）`,
    );
  }
  ...
  if (result.hasSpecial) {
    ...
    if (result.halfDayFee > 0) {
      lines.push(`　・半日加算（午後お迎え）：${yen(result.halfDayFee)}`);
    }
  }

  lines.push(`💰 概算料金：${yen(result.total ?? 0)}（税込）`);
  if (result.hasSpecial) {
